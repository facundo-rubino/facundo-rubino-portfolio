...
export const FloatingNav = ({
  ...
  useMotionValueEvent(scrollYProgress, "change", (current) => {
    if (typeof current === "number") {
      ...
      if (current < 0.05) {
        setVisible(true); // Show navbar if at the top of the page
      } else {
        if (direction < 0) {
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        ...
        className={cn(
          "flex max-w-fit  fixed top-10 inset-x-0 mx-auto border border-transparent dark:border-white/[0.2] rounded-full dark:bg-blue-900 bg-white shadow-[0px_2px_3px_-1px_rgba(0,0,0,0.1),0px_1px_0px_0px_rgba(25,28,33,0.02),0px_0px_0px_1px_rgba(25,28,33,0.08)] z-[5000] pr-2 pl-8 py-2  items-center justify-center space-x-4",
          className
        )}
      >
