...
title:string, icon: React.ReactNode, position: string, handleClick?: () => void,
  ...
  <span className="inline-flex gap-2 h-full w-full cursor-pointer items-center justify-center rounded-lg dark:bg-blue-900 px-3 text-sm font-medium text-white backdrop-blur-3xl">
    {position === 'left' && icon}
    {title}
    {position === 'right' && icon}
  </span>
